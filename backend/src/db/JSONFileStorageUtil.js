...
class JSONFileStorageUtil {
    ...
    initialise() {
        ...
        fs.access(this.filename,fs.constants.F_OK, (err) => {
            if (!err) {
                ...
                let dbFile = fs.createWriteStream(this.filename, {flags: 'w'});
    ...
    saveWithId(key,saveData) {
        ...
        let previousResults = this.getDataFromFile();
        let foundIndex = previousResults.findIndex((element) => element.id === key);
        if (foundIndex >= 0) {
    ...
    getWithId(key) {
        ...
        let foundIndex = this.savedData.findIndex((element) => element.id === key);
        if (foundIndex >= 0) {
            debug("Found item");
